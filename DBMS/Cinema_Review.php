...
if(!$conn)
{
 ...
 $stid = oci_parse($conn, 'SELECT USERNAME,COMMENTS,RATING,REVIEW_TIMESTAMP FROM CINEMA_REVIEW,USERS  WHERE CINE_ID=:d and CUST_ID=USERID order by RATING DESC');
 $stid3 = oci_parse($conn, 'SELECT ROUND(AVG(RATING),2) AS AVERATING,COUNT(RATING) AS RATINGNUM,COUNT(COMMENTS) AS COMMENTSNUM FROM CINEMA_REVIEW WHERE CINE_ID= :d');
