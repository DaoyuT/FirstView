...
if(!$conn)
{
 ...
 $stid = oci_parse($conn, 'SELECT COMMENTS,RATING,REVIEW_TIMESTAMP FROM CINEMA_REVIEW  WHERE CINE_ID=:d and CUST_ID=:f ');
 $stid3 = oci_parse($conn, 'SELECT COUNT(RATING) AS RATINGNUM,COUNT(COMMENTS) AS COMMENTSNUM FROM CINEMA_REVIEW WHERE CINE_ID= :d and CUST_ID=:f ');
...
{ 
	if($RATINGNUM==0) 
		{echo "You have not rated the cinema.";
		 echo "<br>";}
	else{echo "Your rate for this Cinema:  ";
	switch ($RATING) {
    case 1:
	...
	if($COMMENTSNUM==0) 
	{echo "You have not commented on the cinema.";
	 echo "<br>";}
	 else{ echo "Your comment on this Cinema:  ";
	 echo "<br>";
	 echo $COMMENTS;
	 echo "<br>";
	 echo "<br>";	 
...
<div id="Contents1">
<form id="review" action="My_cinema_review_submit.php" method="get">
<ul>
<h5>
...
echo" <input type='hidden' name='cinemaid' value='$temp'/>";
